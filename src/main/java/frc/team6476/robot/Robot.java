...
public class Robot extends IterativeRobot {
    ...
    public void autonomousPeriodic() {
        ...
        drivetrain.driveTime(2.0,Constants.autoSpeed);
        publishStats();
    }
    ...
    public void teleopPeriodic() {
        ...
        else if (xboxController.getY(GenericHID.Hand.kRight)>0.1)
        {
            double speed = xboxController.getY(GenericHID.Hand.kRight);
            if(speed > Constants.liftDownSpeed) {
                speed = Constants.liftDownSpeed;
            }
            lift.lower(speed);
        }
        else if (xboxController.getY(GenericHID.Hand.kRight)<-0.1)
        {
            double speed = xboxController.getY(GenericHID.Hand.kRight);
            if (speed < -Constants.liftUpSpeed) {
                speed = -Constants.liftUpSpeed;
            }
            lift.raise(-speed);
        }
